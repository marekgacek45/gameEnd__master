...
@csrf
@method('DELETE')
                                    <button type="submit" class="border px-8 py-4">{{$tag->id}}</button>
                                </form>
...
<div class="relative w-full max-w-lg max-h-full">
    ...
    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        <!-- Modal body -->
        <div class="p-6 space-y-6">
            <form action="{{ route('admin.tag.update',$tag->id) }}" method="post" id="updateTag">
                @csrf
                @method('patch')
                <p>id={{$tag->id}}</p>
                <x-form.input name="name" namePl="Nazwa Kategori" class=" w-full xl:w-3/4 "
        ...
        <!-- Modal footer -->
        <div class="flex items-center p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
            <button form="updateTag" data-modal-hide="modal-edit" type="submit"
